...
- GSC access is read-only. The Environment profile supplies `GSC_TOKEN_JSON` directly to the repo-local client. Legacy explicit file consumers may still use `GSC_TOKEN_FILE` until migrated. `scripts/gsc-mcp` reads its variables from the `Zush MCP` 1Password Environment mounted at `.secrets/mcp.env` (existing shared `../zush-app/.secrets/mcp.env` is used only when the local mount is absent) (override with `ZUSH_MCP_ENV_FILE`); it does not run `op` at startup. Never commit OAuth tokens or client secrets.
