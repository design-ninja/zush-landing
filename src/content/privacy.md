...
- **Bring Your Own Key (BYOK):** Users on every plan may configure their own API keys for Groq, Google Gemini, OpenAI, or Anthropic Claude. BYOK keys are stored locally in secure platform storage. In BYOK mode, the key is sent to Zush backend and the selected AI provider only to validate the key and process AI requests. Zush backend does not permanently store BYOK API keys.
...
- **AI Providers:** Groq, Google Gemini, and [Alibaba Cloud Model Studio (Qwen)](https://www.alibabacloud.com/help/en/model-studio/privacy-notice). With BYOK: optionally OpenAI or Anthropic Claude.
