...
description: "An AI document renamer reads what is inside a PDF, DOCX, or XLSX and names it from that. How it works on Mac, and where it beats find-and-replace renaming."
date: "2026-03-23"
...
Quick answer: an AI document renamer reads the actual content of PDFs, DOCX, XLSX, and similar files — invoice numbers, contract parties, headings — and replaces generic names like `document (3).pdf` with descriptive, searchable filenames automatically.
...
<BlogProductIntro
  ...
  description="Zush reads PDFs, DOCX, and XLSX files and names them by what is inside, so invoices, reports, and contracts arrive with names you can search."
  links={[
...
[Zush](https://zushapp.com/rename-documents-with-ai) supports 37 document and PDF-related formats:
...
Zush offers 50 free renames, which is enough to test the workflow on a real folder. If document renaming becomes part of your regular routine, PRO unlocks unlimited renames with either a $10/month plan or a $48 one-time plan. For heavy or automated workflows, BYOK remains available if you want to use your own AI provider key.
