...
[Zush](/) is a native macOS app built specifically for AI-powered file renaming. Our main reason for putting it first is format breadth: 91 supported formats across images, documents, and videos. That includes HEIC, AVIF, PSD, SVG, RAW camera files, PDFs, Office files, OpenDocument files, subtitle files, and common video formats like MP4 and MOV. Real Mac workflows mix iPhone photos, camera RAW files, scanned PDFs, office documents, subtitles, and screen recordings in the same folders, and most tools we tested failed on at least one of those categories.
