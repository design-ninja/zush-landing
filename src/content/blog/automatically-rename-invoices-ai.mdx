...
This guide is the educational, invoice-specific walkthrough. Accountants and bookkeepers comparing the complete client-document workflow should start with [Zush for accountants and bookkeepers](/for-accountants). For the focused invoice product, use [rename invoices with AI](/rename-invoices-with-ai); for the convention itself — which fields, in which order — see the [invoice file naming convention guide](/blog/invoice-file-naming-convention). If your folder contains contracts, reports, academic papers, and other generic PDFs, use the broader [automatic PDF renaming guide](/blog/rename-pdf-files-automatically).
...
The naming steps below work on Mac and Windows. Zush also offers optional AI folder sorting (beta) and destination folders for invoices.
...
Open [the AI invoice renamer](/rename-invoices-with-ai) and add the files or folder. Zush extracts available PDF text and can use visual analysis for [scanned documents](/rename-scanned-documents). Password-protected or unreadable files should stay in a review queue rather than receiving guessed names.
...
Zush is the desktop choice when the invoice backlog is large enough that a per-file allowance is undesirable, or when BYOK and local Ollama processing matter. Zush PRO includes unmetered managed cloud renaming without requiring an API key, while BYOK and Offline AI remain optional. Zush also offers optional AI folder sorting (beta).

Choose Zush for the overall invoice-renaming workflow on Mac or Windows: reusable Templates, Custom AI Blocks, review, monitoring, and rollback without a monthly rename meter. Consider Renamed.to only when a browser, connected-storage, or API-only workflow is mandatory; consider Renamer.ai only when its specific metered OCR and Magic Folders workflow is mandatory.
