...
Quick answer: free AI file renaming on Mac and Windows works through three paths — Zush free tier (50 renames), the open-source [`ozgrozer/ai-renamer`](https://github.com/ozgrozer/ai-renamer) CLI with Ollama for unlimited local processing, or BYOK with a free-tier API like Google Gemini for cloud quality at near-zero cost.
...
<BlogProductIntro
  title="Want to try AI renaming without paying?"
  description="Zush includes 50 free renames with no signup, and BYOK mode lets you keep going on your own API key instead of a subscription."
  links={[
...
[Zush](/) includes 50 free renames. No credit card, no trial period, no feature restrictions. You get the same AI-powered content analysis, batch rename, folder monitoring, and undo capabilities as paid users.
...
The cost per file rename through BYOK is provider-dependent and changes with model, file size, prompt length, and token usage. In practice it can be fractions of a cent per file on efficient models, but confirm current provider pricing before estimating a large batch. For a deeper cost walkthrough, see [BYOK: Unlimited AI File Renames with Your Own API Key](/blog/byok-ai-file-renaming-unlimited).
...
For a broader comparison of tools and pricing, see [AI renamer tools comparison](/blog/best-ai-file-renamer-tools-2026).

## Option 3: AI Renamer (CLI + Desktop app)

Two unrelated projects share this name, and only one of them is free. [`ozgrozer/ai-renamer`](https://github.com/ozgrozer/ai-renamer) is a GPL-3.0 Node.js CLI on GitHub that runs AI models locally through Ollama -- completely free, with no API calls leaving your computer. The commercial desktop app at airenamer.app is a different product: it offers a GUI with local and cloud options and gives 10 free credits on signup, but its pricing page lists no CLI and no open-source tier.

### CLI setup process

1. Install Ollama from [ollama.ai](https://ollama.ai).
2. Download a vision-capable model such as `qwen2.5vl:3b`, `gemma3:4b`, or another model your hardware can run comfortably.
3. Install the `ai-renamer` CLI via npm (`npx ai-renamer /path` also works).
...
AI Renamer works if privacy is your top priority. The free GitHub CLI is best for developers comfortable with the terminal; the $19 desktop app is easier to use and markets broader format support, but it is a paid product, not the open-source one. For the direct trade-off against Zush, read [Zush vs AI Renamer](/blog/zush-vs-airenamer). If you are choosing between free cloud credits, BYOK, and local models, read [Cloud AI vs Local AI File Renaming](/blog/cloud-vs-local-ai-file-renaming).
...
2. Riffo (riffo.ai): download the free Mac app or use the web demo. The homepage says it is free and publishes no pricing page, so there is also no stated usage cap either way.
...
- **Heavy use (500+ files/month):** Zush PRO unlocks unlimited renames with a $10/month plan or a $48 one-time plan. BYOK is optional when you want provider control.

The progression from free to paid is smooth. You do not lose any features or need to reconfigure anything.

For setup guides on configuring BYOK providers, see [BYOK Setup Guide](/docs/byok).
