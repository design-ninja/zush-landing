...
Quick answer: Zush is <BlogHighlight>$10 one-time</BlogHighlight> for Mac and Windows with 91 supported formats, including RAW photos, documents, and videos. NameQuick is <BlogHighlight>macOS only</BlogHighlight>, priced at $5/month (Managed) or $38 one-time + your own API key (Self-Managed/BYOK), with 7 AI providers and a Rules Engine. Choose Zush for cross-platform + lowest total cost. Choose NameQuick for Mac-native depth.
...
Zush runs on both Mac (macOS 14+) and Windows (10/11) and offers two PRO plans: $8/month or $38 one-time, both with unlimited PRO renames. BYOK remains available when you want to use your own AI provider key. Version 2.0 supports 91 formats: 42 image formats including RAW, HEIC, AVIF, PSD, SVG, and TIFF; 33 document formats including Office, OpenDocument, text, CSV/TSV, JSON, XML/YAML, EML, SRT, and VTT; and 16 video formats including MP4, MOV, M4V, MPEG, 3GP, TS, MTS, M2TS, DV, and VOB.
...
NameQuick mentions audio, video, and archives as part of its 30+ formats. Zush 2.0 supports common video files and subtitle sidecars, but it still does not rename audio-only files or archives. If your folders include a lot of MP3 or ZIP files, NameQuick is broader; if the gap is MOV/MP4 screen recordings and clips, Zush now covers that workflow.
...
- You handle audio-only files or archives that need renaming
