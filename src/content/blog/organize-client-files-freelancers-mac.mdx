...
Quick answer: freelancers should mirror folders per client, then per project, then per phase, with filenames like `clientname-project-phase-asset.ext`. Add a `shared` subfolder per client for assets you reuse, and archive old projects yearly.
...
<BlogProductIntro
  title="Client folders that only you can navigate?"
  description="Zush names contracts, invoices, exports, and reference photos by client and content, so handoffs and year-end searches stop being archaeology."
  links={[
...
![Zush batch rename demo for organizing client files and freelancer project assets](/images/showcase/macos/batch-rename-light.webp)
