...
Quick answer: rename video files with AI by using a tool that can inspect sampled frames and optional subtitle context, then preview names before applying them. In Zush 2.0, MP4, MOV, M4V, MPEG, 3GP, TS, MTS, M2TS, DV, VOB, and related video formats can be renamed alongside photos, screenshots, PDFs, and documents.
...
Zush 2.0 supports 16 common video formats:
...
Audio-only formats are not part of the supported file list yet. If your folder is mostly MP3, WAV, or podcast audio, you still need an audio-specific workflow.
...
Zush can process mixed folders, so your video folder can include `.mp4`, `.mov`, `.m2ts`, `.srt`, `.vtt`, screenshots, PDFs, and notes. Batch review supports up to 50 files at a time.
...
Not yet. Zush 2.0 adds video support, but audio-only formats are still outside the supported file list.
