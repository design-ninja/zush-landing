...
import { REAL_ESTATE_FIELDS } from '@/data/realEstateLanding';
...
export const RealEstateFieldExplorer = () => {
  const [selected, setSelected] = useState(0);
  const field = REAL_ESTATE_FIELDS[selected];
  ...
  return (
    <div className={styles.Explorer}>
      ...
      <div className={styles.Explorer__Chips} role="tablist" aria-label="Real estate fields Zush reads">
        {REAL_ESTATE_FIELDS.map((item, index) => (
          <button
            key={item.label}
            type="button"
            role="tab"
      ...
      <p className={styles.Explorer__Hint}>
        ...
        with LM Studio or Ollama, or through the provider account your brokerage controls with BYOK.
      </p>
    </div>
  );
};
