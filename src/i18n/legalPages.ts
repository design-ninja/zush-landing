...
Zush est un utilitaire desktop pour macOS et Windows conçu pour aider les utilisateurs à organiser, renommer et gérer les métadonnées de fichiers avec l’intelligence artificielle (IA). Les fonctions incluent le renommage automatique d’images, de vidéos et de documents pris en charge, y compris les PDF, le tagging intelligent des métadonnées (tags Finder sur macOS, propriétés de fichiers sur Windows et métadonnées Spotlight), la surveillance de dossiers avec exclusions par type de fichier, des contrôles optionnels de prompts IA personnalisés pour le renommage et le tagging, le renommage depuis le menu contextuel du Finder sur macOS, le traitement cloud BYOK et le mode IA hors ligne avec des modèles locaux privés via Ollama.
...
Zush は、人工知能（AI）を使ってファイルの整理、リネーム、メタデータ管理を支援する macOS および Windows 向けデスクトップユーティリティです。機能には、画像、動画、PDF を含む対応ドキュメントの自動リネーム、スマートメタデータタグ付け（macOS の Finder タグ、Windows のファイルプロパティ、Spotlight メタデータ）、ファイルタイプ除外付きのフォルダ監視、リネームおよびタグ付け動作のための任意のカスタム AI プロンプト制御、macOS の Finder コンテキストメニューからのリネーム、BYOK クラウド処理、Ollama 経由のプライベートなローカルモデルによるオフライン AI モードが含まれます。
...
Zush는 인공지능(AI)을 사용해 사용자가 파일을 정리, 이름 변경, 메타데이터 관리할 수 있도록 설계된 macOS 및 Windows용 데스크톱 유틸리티입니다. 기능에는 이미지, 비디오, PDF를 포함한 지원 문서 자동 이름 변경, 스마트 메타데이터 태그(Finder 태그, Windows 파일 속성, Spotlight 메타데이터), 파일 유형 제외가 포함된 폴더 모니터링, 이름 변경 및 태그 동작을 위한 선택적 사용자 지정 AI 프롬프트 제어, macOS Finder 컨텍스트 메뉴 이름 변경, BYOK 클라우드 처리, Ollama를 통한 개인 로컬 모델 기반 오프라인 AI 모드가 포함됩니다.
...
Zush 是一款适用于 macOS 和 Windows 的桌面工具，旨在帮助用户使用人工智能（AI）整理、重命名和管理文件元数据。功能包括对图片、视频和支持的文档（包括 PDF）进行自动重命名、智能元数据标签（macOS 上的 Finder 标签、Windows 上的文件属性以及 Spotlight 元数据）、带文件类型排除的文件夹监控、用于重命名和标签行为的可选自定义 AI 提示词控制、macOS Finder 右键菜单重命名、BYOK 云端处理，以及通过 Ollama 的私有本地模型实现的离线 AI 模式。
