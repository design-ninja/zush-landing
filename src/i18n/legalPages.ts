...
const LEGAL_MARKDOWN: Partial<Record<InternalLocale, LegalMarkdownByRoute>> = {
  de: {
    '/privacy-policy': `Diese Datenschutzerklärung erklärt, wie Zush ("Kirill Isachenko", "wir", "uns" oder "unser") deine Informationen erfasst, nutzt und schützt, wenn du unsere macOS- und Windows-Apps sowie zugehörige Dienste verwendest.
...
- Lizenzstatus: Informationen über deine PRO-Lizenz oder dein PRO-Abonnement.
- Nutzungszähler: Wir erfassen die Anzahl KI-verarbeiteter Dateien, um Nutzungslimits zu verwalten.

**1.5 Zahlungsinformationen**
Wir speichern oder verarbeiten deine Kreditkartendaten nicht. Alle Zahlungen werden sicher von unseren Zahlungsdienstleistern abgewickelt: [Paddle.com](https://www.paddle.com) (für Direktkäufe), [Apple](https://www.apple.com/legal/privacy/) (für Mac-App-Store-Käufe) und [Microsoft](https://privacy.microsoft.com/privacystatement) (für Microsoft-Store-Käufe). Bitte lies deren Datenschutzerklärungen für Details.

Bei Store-Käufen sendet Zush Store-Produkt- und Transaktionskennungen an das Zush-Backend, um PRO-Berechtigungen zu verifizieren, Käufe wiederherzustellen, Missbrauch zu verhindern und den Kontostatus zu pflegen.
...
- **Bring Your Own Key (BYOK):** Nutzer aller Pläne können eigene API-Schlüssel für Google Gemini, OpenRouter, OpenAI oder Anthropic Claude konfigurieren. BYOK-Schlüssel werden lokal im sicheren Systemspeicher abgelegt. Im BYOK-Modus wird der Schlüssel nur zur Validierung und Verarbeitung von KI-Anfragen an das Zush-Backend und den ausgewählten KI-Anbieter gesendet. Bei OpenRouter können sowohl OpenRouter als auch der Anbieter des ausgewählten Modells die Anfrage verarbeiten. Das Zush-Backend speichert BYOK-API-Schlüssel nicht dauerhaft.
...
- **KI-Anbieter:** Groq, Google Gemini und [Alibaba Cloud Model Studio (Qwen)](https://www.alibabacloud.com/help/en/model-studio/privacy-notice). Mit BYOK: Google Gemini, OpenRouter und dessen Modellanbieter, OpenAI oder Anthropic Claude.
...
- **Nutzungsanalyse:** TelemetryDeck (datenschutzorientierte Produktanalyse) und PostHog (datenschutzfreundliche Website-Analyse).
...
Zush ist ein Desktop-Dienstprogramm für macOS und Windows, das Nutzern hilft, Dateien mithilfe künstlicher Intelligenz (KI) zu organisieren, umzubenennen und Metadaten zu verwalten. Zu den Funktionen gehören automatische Umbenennung für Bilder, Design-Dateien, Audio, Videos und unterstützte Dokumente einschließlich PDFs und iWork-Dateien, wiederverwendbare Templates, 145+ Naming Blocks, smarte Metadaten-Tags (Finder-Tags auf macOS, Dateieigenschaften auf Windows und Spotlight-Metadaten), Ordnerüberwachung mit Dateityp-Ausschlüssen, optionale benutzerdefinierte KI-Prompt-Steuerung für Umbenennung und Tagging, Umbenennen über das Finder-Kontextmenü auf macOS, BYOK-Cloud-Verarbeitung und Offline-KI-Modus mit privaten lokalen Modellen via Ollama.
...
Käufe über den Mac App Store oder Microsoft Store werden von der jeweiligen Store-Plattform gemäß deren Zahlungs-, Rückerstattungs- und Kontobedingungen verarbeitet.
...
Zush PRO ist als Monatsplan oder Einmalkauf verfügbar. Wir bieten eine 14-tägige Geld-zurück-Garantie ab Kaufdatum.
...
Wir behalten uns das Recht vor, diese Rückerstattungsrichtlinie jederzeit zu ändern. Änderungen werden auf dieser Seite mit einem aktualisierten Datum "Zuletzt aktualisiert" veröffentlicht.`,
  ...
  fr: {
    '/privacy-policy': `Cette Politique de confidentialité explique comment Zush ("Kirill Isachenko", "nous", "notre" ou "nos") collecte, utilise et protège vos informations lorsque vous utilisez nos applications macOS et Windows et les services associés.
...
Vos fichiers originaux en pleine résolution ne quittent pas votre appareil dans le fonctionnement cloud normal, sauf lorsqu’une charge audio limitée est nécessaire à la transcription ou lorsqu’un fichier iWork doit être temporairement converti pour l’analyse. Si vous activez le mode IA hors ligne, l’analyse des fichiers pris en charge s’exécute via des modèles locaux privés avec Ollama plutôt que via le cloud Zush ou des fournisseurs IA tiers.
...
- Statut de licence : informations sur votre licence ou abonnement PRO.
- Compteurs d’usage : nous suivons le nombre de fichiers traités par IA pour gérer les limites.

**1.5 Informations de paiement**
Nous ne stockons ni ne traitons vos données de carte bancaire. Tous les paiements sont traités de manière sécurisée par nos prestataires de paiement : [Paddle.com](https://www.paddle.com) (achats directs), [Apple](https://www.apple.com/legal/privacy/) (achats Mac App Store) et [Microsoft](https://privacy.microsoft.com/privacystatement) (achats Microsoft Store). Consultez leurs politiques de confidentialité respectives pour plus de détails.

Pour les achats en store, Zush envoie les identifiants de produit et de transaction du store au backend Zush afin de vérifier l’accès PRO, restaurer les achats, prévenir les abus et maintenir l’état du compte.
...
- **Bring Your Own Key (BYOK) :** les utilisateurs de toutes les offres peuvent configurer leurs propres clés API pour Google Gemini, OpenRouter, OpenAI ou Anthropic Claude. Les clés BYOK sont stockées localement dans le stockage sécurisé du système. En mode BYOK, la clé est envoyée au backend Zush et au fournisseur IA sélectionné uniquement pour valider la clé et traiter les requêtes IA. Avec OpenRouter, OpenRouter et le fournisseur du modèle sélectionné peuvent traiter la requête. Le backend Zush ne stocke pas durablement les clés API BYOK.
...
**Ce qui n’est pas envoyé automatiquement :** les contenus originaux en pleine résolution et les chemins de dossiers locaux ne sont pas envoyés comme champs séparés. Toutefois, si des informations personnelles apparaissent dans un aperçu, un aperçu design, du texte extrait, une transcription, une charge audio, un aperçu iWork temporaire ou un prompt personnalisé, elles peuvent être traitées dans le cadre de la requête.

**Conservation des données :** les aperçus, aperçus design, textes extraits, aperçus PDF temporaires iWork, transcriptions, charges audio, résumés, modèles, blocs de nommage et prompts personnalisés sont traités pour l’analyse en temps réel. Zush ne stocke pas le contenu des fichiers après traitement dans le fonctionnement normal et ne stocke pas intentionnellement les prompts personnalisés comme fonctionnalité de l’application. Les données envoyées peuvent toutefois être traitées ou conservées par les fournisseurs IA tiers selon leurs propres conditions. Vos contenus de fichiers et prompts ne sont pas utilisés par Zush pour entraîner nos propres modèles.

### 4. Services tiers

Nous partageons des données avec les prestataires suivants :

- **Fournisseurs IA :** Groq, Google Gemini et [Alibaba Cloud Model Studio (Qwen)](https://www.alibabacloud.com/help/en/model-studio/privacy-notice). Avec BYOK : Google Gemini, OpenRouter et ses fournisseurs de modèles, OpenAI ou Anthropic Claude.
...
- **Analyse d’usage :** TelemetryDeck (analytics produit axés sur la confidentialité) et PostHog (analyse web respectueuse de la confidentialité).
...
Zush est un utilitaire desktop pour macOS et Windows conçu pour aider les utilisateurs à organiser, renommer et gérer les métadonnées de fichiers avec l’intelligence artificielle (IA). Les fonctions incluent le renommage automatique d’images, de fichiers design, d’audio, de vidéos et de documents pris en charge, y compris les PDF et fichiers iWork, des modèles réutilisables, 145+ blocs de nommage, le tagging intelligent des métadonnées (tags Finder sur macOS, propriétés de fichiers sur Windows et métadonnées Spotlight), la surveillance de dossiers avec exclusions par type de fichier, des contrôles optionnels de prompts IA personnalisés pour le renommage et le tagging, le renommage depuis le menu contextuel du Finder sur macOS, le traitement cloud BYOK et le mode IA hors ligne avec des modèles locaux privés via Ollama.
...
Les achats effectués via le Mac App Store ou le Microsoft Store sont traités par la plateforme de store concernée selon ses propres conditions de paiement, de remboursement et de compte.
...
Zush PRO est disponible en abonnement mensuel ou achat unique. Nous offrons une garantie de remboursement de 14 jours à compter de la date d’achat.
...
Nous nous réservons le droit de modifier cette politique de remboursement à tout moment. Toute modification sera publiée sur cette page avec une date "Dernière mise à jour" actualisée.`,
  ...
  'pt-br': {
    '/privacy-policy': `Esta Política de Privacidade explica como o Zush ("Kirill Isachenko", "nós", "nos" ou "nosso") coleta, usa e protege suas informações quando você usa nossos aplicativos para macOS e Windows e serviços relacionados.
...
- Status da licença: informações sobre sua licença ou assinatura PRO.
...
Não armazenamos nem processamos os dados do seu cartão de crédito. Todos os pagamentos são processados com segurança pelos nossos processadores: [Paddle.com](https://www.paddle.com) (compras diretas), [Apple](https://www.apple.com/legal/privacy/) (compras na Mac App Store) e [Microsoft](https://privacy.microsoft.com/privacystatement) (compras na Microsoft Store). Consulte as políticas de privacidade deles para mais detalhes.

Para compras em lojas, o Zush envia identificadores de produto e transação da loja ao backend do Zush para verificar o direito ao PRO, restaurar compras, prevenir abuso e manter o status da conta.
...
- **Bring Your Own Key (BYOK):** usuários de todos os planos podem configurar suas próprias chaves de API para Google Gemini, OpenRouter, OpenAI ou Anthropic Claude. As chaves BYOK são armazenadas localmente no armazenamento seguro do sistema. No modo BYOK, a chave é enviada ao backend do Zush e ao provedor selecionado apenas para validar a chave e processar solicitações de IA. Com o OpenRouter, tanto o OpenRouter quanto o provedor do modelo selecionado podem processar a solicitação. O backend do Zush não armazena chaves BYOK permanentemente.
...
- **Provedores de IA:** Groq, Google Gemini e [Alibaba Cloud Model Studio (Qwen)](https://www.alibabacloud.com/help/en/model-studio/privacy-notice). Com BYOK: Google Gemini, OpenRouter e seus provedores de modelos, OpenAI ou Anthropic Claude.
...
- **Análise de uso:** TelemetryDeck (analytics de produto com foco em privacidade) e PostHog (analytics de site com foco em privacidade).
...
Zush é um utilitário desktop para macOS e Windows criado para ajudar usuários a organizar, renomear e gerenciar metadados de arquivos usando inteligência artificial (IA). Os recursos incluem renomeação automática para imagens, arquivos de design, áudio, vídeos e documentos compatíveis, incluindo PDFs e arquivos iWork, modelos reutilizáveis, 145+ blocos de nomes, tags inteligentes de metadados (tags do Finder no macOS, propriedades de arquivo no Windows e metadados do Spotlight), monitoramento de pastas com exclusões por tipo de arquivo, controles opcionais de prompts personalizados para renomeação e tags, renomeação pelo menu de contexto do Finder no macOS, processamento cloud BYOK e modo IA offline com modelos locais privados via Ollama.
...
Compras feitas pela Mac App Store ou Microsoft Store são processadas pela plataforma de loja correspondente conforme seus próprios termos de pagamento, reembolso e conta.
...
Zush PRO está disponível como plano mensal ou compra única. Oferecemos garantia de reembolso de 14 dias a partir da data da compra.
...
Reservamo-nos o direito de modificar esta política de reembolso a qualquer momento. Qualquer alteração será publicada nesta página com uma data "Última atualização" atualizada.`,
  ...
  es: {
    '/privacy-policy': `Esta Política de privacidad explica cómo Zush ("Kirill Isachenko", "nosotros" o "nuestro") recopila, usa y protege tu información cuando usas nuestras aplicaciones para macOS y Windows y los servicios relacionados.
...
- Estado de licencia: información sobre tu licencia o suscripción PRO.
...
No almacenamos ni procesamos los datos de tu tarjeta de crédito. Todos los pagos son procesados de forma segura por nuestros procesadores: [Paddle.com](https://www.paddle.com) (compras directas), [Apple](https://www.apple.com/legal/privacy/) (compras en Mac App Store) y [Microsoft](https://privacy.microsoft.com/privacystatement) (compras en Microsoft Store). Consulta sus políticas de privacidad para más detalles.

Para compras en tiendas, Zush envía identificadores de producto y transacción de la tienda al backend de Zush para verificar el derecho PRO, restaurar compras, prevenir abuso y mantener el estado de cuenta.
...
- **Bring Your Own Key (BYOK):** usuarios de todos los planes pueden configurar sus propias claves API para Google Gemini, OpenRouter, OpenAI o Anthropic Claude. Las claves BYOK se almacenan localmente en el almacenamiento seguro del sistema. En modo BYOK, la clave se envía al backend de Zush y al proveedor seleccionado solo para validar la clave y procesar solicitudes de IA. Con OpenRouter, tanto OpenRouter como el proveedor del modelo seleccionado pueden procesar la solicitud. El backend de Zush no almacena permanentemente claves API BYOK.
...
- **Proveedores de IA:** Groq, Google Gemini y [Alibaba Cloud Model Studio (Qwen)](https://www.alibabacloud.com/help/en/model-studio/privacy-notice). Con BYOK: Google Gemini, OpenRouter y sus proveedores de modelos, OpenAI o Anthropic Claude.
...
- **Analítica de uso:** TelemetryDeck (analítica de producto centrada en privacidad) y PostHog (analítica web centrada en privacidad).
...
Zush es una utilidad de escritorio para macOS y Windows diseñada para ayudar a organizar, renombrar y gestionar metadatos de archivos con inteligencia artificial (IA). Las funciones incluyen renombrado automático para imágenes, archivos de diseño, audio, videos y documentos compatibles, incluidos PDF y archivos iWork, plantillas reutilizables, 145+ bloques de nombres, etiquetado inteligente de metadatos (tags de Finder en macOS, propiedades de archivo en Windows y metadatos Spotlight), monitoreo de carpetas con exclusiones por tipo de archivo, controles opcionales de prompts personalizados para renombrado y etiquetado, renombrado desde el menú contextual de Finder en macOS, procesamiento cloud BYOK y modo IA offline con modelos locales privados mediante Ollama.
...
Las compras realizadas mediante Mac App Store o Microsoft Store son procesadas por la plataforma de tienda correspondiente bajo sus propios términos de pago, reembolso y cuenta.
...
Zush PRO está disponible como plan mensual o compra única. Ofrecemos una garantía de devolución de dinero de 14 días desde la fecha de compra.
...
Nos reservamos el derecho de modificar esta política de reembolso en cualquier momento. Cualquier cambio se publicará en esta página con una fecha "Última actualización" actualizada.`,
  ...
  nl: {
    '/privacy-policy': `Dit privacybeleid legt uit hoe Zush ("Kirill Isachenko", "wij", "ons" of "onze") je informatie verzamelt, gebruikt en beschermt wanneer je onze macOS- en Windows-apps en gerelateerde diensten gebruikt.
...
Je originele bestanden in volledige resolutie verlaten je apparaat niet bij normale cloudwerking, behalve wanneer een begrensde audiopayload nodig is voor transcriptie of wanneer een iWork-bestand tijdelijk moet worden geconverteerd voor analyse. Als je Offline AI inschakelt, wordt ondersteunde bestandsanalyse uitgevoerd via private lokale modellen met Ollama in plaats van Zush cloud of externe AI-providers.
...
- Licentiestatus: informatie over je PRO-licentie of abonnement.
- Gebruiksstatistieken: we volgen het aantal AI-verwerkte bestanden om gebruikslimieten te beheren.

**1.5 Betalingsinformatie**
We slaan je creditcardgegevens niet op en verwerken ze niet. Alle betalingen worden veilig afgehandeld door onze betalingsverwerkers: [Paddle.com](https://www.paddle.com) (directe aankopen), [Apple](https://www.apple.com/legal/privacy/) (Mac App Store-aankopen) en [Microsoft](https://privacy.microsoft.com/privacystatement) (Microsoft Store-aankopen). Raadpleeg hun privacybeleid voor details.

Voor store-aankopen stuurt Zush store-product- en transactie-ID's naar de Zush-backend om PRO-rechten te verifiëren, aankopen te herstellen, misbruik te voorkomen en accountstatus te onderhouden.
...
- **Bring Your Own Key (BYOK):** Gebruikers van elk abonnement kunnen eigen API-sleutels configureren voor Google Gemini, OpenRouter, OpenAI of Anthropic Claude. BYOK-sleutels worden lokaal opgeslagen in de beveiligde systeemopslag. In BYOK-modus wordt de sleutel alleen naar de Zush-backend en de gekozen AI-provider verzonden om de sleutel te valideren en AI-verzoeken te verwerken. Met OpenRouter kunnen zowel OpenRouter als de provider van het gekozen model het verzoek verwerken. De Zush-backend slaat BYOK API-sleutels niet permanent op.
...
**Wat niet automatisch wordt verzonden:** originele inhoud in volledige resolutie en lokale maplocaties worden niet als aparte velden verzonden. Als persoonlijke informatie echter voorkomt in een preview, designpreview, geëxtraheerde tekst, transcript, audiopayload, tijdelijke iWork-preview of aangepaste prompt, kan die informatie als onderdeel van het verzoek worden verwerkt.

**Gegevensbewaring:** previews, designpreviews, geëxtraheerde tekst, tijdelijke iWork-PDF-previews, transcripties, audiopayloads, samenvattingen, templates, Naming Blocks en aangepaste prompts worden verwerkt voor realtime analyse. Zush slaat bestandsinhoud na verwerking niet op als onderdeel van normale werking en slaat aangepaste prompts niet bewust op als appfunctie. Ingezonden gegevens kunnen nog wel door externe AI-providers worden verwerkt of bewaard volgens hun eigen voorwaarden. Je bestandsinhoud en prompttekst worden niet door Zush gebruikt om onze eigen modellen te trainen.

### 4. Derde partijen

We delen gegevens met de volgende dienstverleners:

- **AI-providers:** Groq, Google Gemini en [Alibaba Cloud Model Studio (Qwen)](https://www.alibabacloud.com/help/en/model-studio/privacy-notice). Met BYOK: Google Gemini, OpenRouter en zijn modelproviders, OpenAI of Anthropic Claude.
...
- **Gebruiksanalyse:** TelemetryDeck (privacygerichte productanalytics) en PostHog (privacyvriendelijke websiteanalytics).
...
Zush is een desktoptool voor macOS en Windows die gebruikers helpt bestanden te organiseren, hernoemen en metadata te beheren met kunstmatige intelligentie (AI). Functies omvatten automatische hernoeming voor afbeeldingen, designbestanden, audio, video’s en ondersteunde documenten inclusief PDF's en iWork-bestanden, herbruikbare templates, 145+ Naming Blocks, slimme metadatatags (Finder-tags op macOS, bestandseigenschappen op Windows en Spotlight-metadata), mapbewaking met uitsluitingen per bestandstype, optionele aangepaste AI-promptinstellingen voor hernoeming en tagging, hernoemen via het Finder-contextmenu op macOS, BYOK-cloudverwerking en Offline AI met private lokale modellen via Ollama.
...
Aankopen via de Mac App Store of Microsoft Store worden verwerkt door het betreffende storeplatform volgens de eigen betalings-, terugbetalings- en accountvoorwaarden.
...
Zush PRO is beschikbaar als maandplan of eenmalige aankoop. We bieden een geld-terug-garantie van 14 dagen vanaf je aankoopdatum.
...
We behouden ons het recht voor dit terugbetalingsbeleid op elk moment te wijzigen. Wijzigingen worden op deze pagina geplaatst met een bijgewerkte datum "Laatst bijgewerkt".`,
  ...
  it: {
    '/privacy-policy': `Questa Informativa sulla privacy spiega come Zush ("Kirill Isachenko", "noi" o "nostro") raccoglie, usa e protegge le tue informazioni quando usi le nostre applicazioni per macOS e Windows e i servizi correlati.
...
- Stato licenza: informazioni sulla licenza o abbonamento PRO.
- Conteggi d’uso: tracciamo il numero di file elaborati con IA per gestire i limiti.

**1.5 Informazioni di pagamento**
Non memorizziamo né elaboriamo i dati della tua carta di credito. Tutti i pagamenti sono gestiti in modo sicuro dai nostri processori: [Paddle.com](https://www.paddle.com) (acquisti diretti), [Apple](https://www.apple.com/legal/privacy/) (acquisti Mac App Store) e [Microsoft](https://privacy.microsoft.com/privacystatement) (acquisti Microsoft Store). Consulta le rispettive informative privacy per dettagli.

Per gli acquisti negli store, Zush invia identificatori di prodotto e transazione dello store al backend Zush per verificare il diritto PRO, ripristinare acquisti, prevenire abusi e mantenere lo stato dell’account.
...
- **Bring Your Own Key (BYOK):** gli utenti di tutti i piani possono configurare le proprie chiavi API per Google Gemini, OpenRouter, OpenAI o Anthropic Claude. Le chiavi BYOK sono archiviate localmente nello spazio sicuro del sistema. In modalità BYOK, la chiave viene inviata al backend Zush e al provider IA selezionato solo per validare la chiave ed elaborare le richieste IA. Con OpenRouter, sia OpenRouter sia il provider del modello selezionato possono elaborare la richiesta. Il backend Zush non memorizza permanentemente le chiavi API BYOK.
...
- **Provider IA:** Groq, Google Gemini e [Alibaba Cloud Model Studio (Qwen)](https://www.alibabacloud.com/help/en/model-studio/privacy-notice). Con BYOK: Google Gemini, OpenRouter e i relativi provider di modelli, OpenAI o Anthropic Claude.
...
- **Analytics d’uso:** TelemetryDeck (analytics prodotto orientate alla privacy) e PostHog (analytics del sito orientate alla privacy).
...
Zush è un’utilità desktop per macOS e Windows progettata per aiutare gli utenti a organizzare, rinominare e gestire metadati dei file usando intelligenza artificiale (IA). Le funzioni includono rinomina automatica per immagini, file design, audio, video e documenti supportati, inclusi PDF e file iWork, modelli riutilizzabili, 145+ blocchi di nomi, tagging intelligente dei metadati (tag Finder su macOS, proprietà file su Windows e metadati Spotlight), monitoraggio cartelle con esclusioni per tipo di file, controlli opzionali di prompt IA personalizzati per rinomina e tagging, rinomina dal menu contestuale del Finder su macOS, elaborazione cloud BYOK e modalità IA offline con modelli locali privati via Ollama.
...
Gli acquisti effettuati tramite Mac App Store o Microsoft Store sono elaborati dalla piattaforma store applicabile secondo i propri termini di pagamento, rimborso e account.
...
Zush PRO è disponibile come piano mensile o acquisto una tantum. Offriamo una garanzia di rimborso di 14 giorni dalla data di acquisto.
...
Ci riserviamo il diritto di modificare questa politica di rimborso in qualsiasi momento. Ogni modifica sarà pubblicata su questa pagina con una data "Ultimo aggiornamento" aggiornata.`,
  ...
  ja: {
    '/privacy-policy': `このプライバシーポリシーは、Zush（"Kirill Isachenko"、"当社"、"私たち"）が、macOS および Windows アプリケーションと関連サービスの利用時に、どのように情報を収集、使用、保護するかを説明します。
...
通常のクラウド動作では、文字起こしに必要な限定的な音声ペイロードや iWork ファイルの一時変換が必要な場合を除き、元のフル解像度ファイルがデバイス外へ送信されることはありません。オフライン AI モードを有効にした場合、対応ファイルの分析は Zush クラウドや第三者 AI プロバイダーではなく、Ollama 経由のプライベートなローカルモデルで実行されます。
...
- メールアドレス: 購入時に提供されます。
- ライセンス状態: PRO ライセンスまたはサブスクリプションに関する情報。
- 利用回数: 利用制限を管理するため、AI で処理されたファイル数を追跡します。

**1.5 支払い情報**
当社はクレジットカード情報を保存または処理しません。すべての支払いは、決済処理業者である [Paddle.com](https://www.paddle.com)（直接購入）、[Apple](https://www.apple.com/legal/privacy/)（Mac App Store 購入）、[Microsoft](https://privacy.microsoft.com/privacystatement)（Microsoft Store 購入）によって安全に処理されます。詳細は各社のプライバシーポリシーをご確認ください。

ストア購入の場合、Zush は PRO 権限の確認、購入復元、不正防止、アカウント状態の維持のため、ストアの製品および取引識別子を Zush バックエンドに送信します。
...
- **Bring Your Own Key (BYOK):** すべてのプランのユーザーは Google Gemini、OpenRouter、OpenAI、Anthropic Claude 用の独自 API キーを設定できます。BYOK キーはシステムの安全なストレージにローカル保存されます。BYOK モードでは、キーの検証と AI リクエスト処理に必要な場合にのみ、Zush バックエンドおよび選択された AI プロバイダーへ送信されます。OpenRouter では、OpenRouter と選択したモデルの提供元の両方がリクエストを処理する場合があります。Zush バックエンドは BYOK API キーを永続保存しません。
...
**自動的に送信されない内容:** 元のフル解像度ファイル内容およびローカルフォルダパスは個別フィールドとして送信されません。ただし、個人情報がプレビュー、デザインプレビュー、抽出テキスト、文字起こし、音声ペイロード、一時的な iWork プレビュー、またはカスタムプロンプト内に含まれる場合、その情報はリクエストの一部として処理される可能性があります。

**データ保持:** プレビュー、デザインプレビュー、抽出テキスト、一時的な iWork PDF プレビュー、文字起こし、音声ペイロード、要約、テンプレート、Naming Blocks、カスタムプロンプトはリアルタイム分析のために処理されます。Zush は通常動作の一部として処理後のファイル内容を保存せず、カスタムプロンプトをアプリ機能として意図的に保存しません。送信データは第三者 AI プロバイダーの条件に従って処理または保持される場合があります。ファイル内容およびプロンプトテキストは、Zush が自社モデルの学習に使用することはありません。

### 4. 第三者サービス

当社は以下のサービス提供者とデータを共有します。

- **AI プロバイダー:** Groq、Google Gemini、および [Alibaba Cloud Model Studio（Qwen）](https://www.alibabacloud.com/help/en/model-studio/privacy-notice)。BYOK では Google Gemini、OpenRouter とそのモデル提供元、OpenAI、Anthropic Claude を利用できます。
...
- **利用分析:** TelemetryDeck（プライバシー重視のプロダクト分析）および PostHog（プライバシーに配慮したウェブサイト分析）。
...
Zush は、人工知能（AI）を使ってファイルの整理、リネーム、メタデータ管理を支援する macOS および Windows 向けデスクトップユーティリティです。機能には、画像、デザインファイル、音声、動画、PDF と iWork を含む対応ドキュメントの自動リネーム、再利用可能なテンプレート、145+ Naming Blocks、スマートメタデータタグ付け（macOS の Finder タグ、Windows のファイルプロパティ、Spotlight メタデータ）、ファイルタイプ除外付きのフォルダ監視、リネームおよびタグ付け動作のための任意のカスタム AI プロンプト制御、macOS の Finder コンテキストメニューからのリネーム、BYOK クラウド処理、Ollama 経由のプライベートなローカルモデルによるオフライン AI モードが含まれます。
...
Mac App Store または Microsoft Store での購入は、該当するストアプラットフォームの支払い、返金、アカウント条件に従って処理されます。
...
Zush PRO は月額プランまたは買い切り購入として利用できます。購入日から 14 日間の返金保証を提供します。
...
当社はこの返金ポリシーをいつでも変更する権利を留保します。変更は、このページに更新された "最終更新日" とともに掲載されます。`,
  ...
  ko: {
    '/privacy-policy': `이 개인정보 처리방침은 Zush("Kirill Isachenko", "당사", "우리")가 macOS 및 Windows 애플리케이션과 관련 서비스를 사용할 때 정보를 수집, 사용, 보호하는 방식을 설명합니다.
...
- 라이선스 상태: PRO 라이선스 또는 구독에 대한 정보.
- 사용 횟수: 사용 제한 관리를 위해 AI 처리 파일 수를 추적합니다.

**1.5 결제 정보**
당사는 신용카드 정보를 저장하거나 처리하지 않습니다. 모든 결제는 결제 처리자인 [Paddle.com](https://www.paddle.com)(직접 구매), [Apple](https://www.apple.com/legal/privacy/)(Mac App Store 구매), [Microsoft](https://privacy.microsoft.com/privacystatement)(Microsoft Store 구매)을 통해 안전하게 처리됩니다. 자세한 내용은 각 개인정보 처리방침을 참고하세요.

스토어 구매의 경우 Zush는 PRO 권한 확인, 구매 복원, 남용 방지, 계정 상태 유지를 위해 스토어 제품 및 거래 식별자를 Zush 백엔드로 전송합니다.
...
- **Bring Your Own Key (BYOK):** 모든 플랜의 사용자는 Google Gemini, OpenRouter, OpenAI, Anthropic Claude용 자체 API 키를 구성할 수 있습니다. BYOK 키는 시스템의 안전한 저장소에 로컬 저장됩니다. BYOK 모드에서 키는 키 검증 및 AI 요청 처리에 필요한 경우에만 Zush 백엔드와 선택한 AI 제공자에게 전송됩니다. OpenRouter를 사용하면 OpenRouter와 선택한 모델의 제공자가 모두 요청을 처리할 수 있습니다. Zush 백엔드는 BYOK API 키를 영구 저장하지 않습니다.
...
**자동으로 전송되지 않는 항목:** 원본 전체 해상도 파일 콘텐츠와 로컬 폴더 경로는 별도 필드로 전송되지 않습니다. 다만 개인 정보가 파일 미리보기, 디자인 미리보기, 추출 텍스트, 전사, 오디오 payload, 임시 iWork 미리보기 또는 사용자 지정 프롬프트에 포함된 경우 해당 정보는 요청의 일부로 처리될 수 있습니다.

**데이터 보관:** 파일 미리보기, 디자인 미리보기, 추출 텍스트, 임시 iWork PDF 미리보기, 전사, 오디오 payload, 요약, 템플릿, Naming Blocks, 사용자 지정 프롬프트는 실시간 분석을 위해 처리됩니다. Zush는 일반 작업의 일부로 처리 후 파일 콘텐츠를 저장하지 않으며, 사용자 지정 프롬프트를 애플리케이션 기능으로 의도적으로 저장하지 않습니다. 제출된 데이터는 타사 AI 제공자의 자체 약관에 따라 처리 또는 보관될 수 있습니다. 파일 콘텐츠와 프롬프트 텍스트는 Zush가 자체 모델 학습에 사용하지 않습니다.

### 4. 타사 서비스

당사는 다음 서비스 제공자와 데이터를 공유합니다.

- **AI 제공자:** Groq, Google Gemini 및 [Alibaba Cloud Model Studio(Qwen)](https://www.alibabacloud.com/help/en/model-studio/privacy-notice). BYOK 사용 시 Google Gemini, OpenRouter 및 해당 모델 제공자, OpenAI 또는 Anthropic Claude를 사용할 수 있습니다.
...
- **사용 분석:** TelemetryDeck(개인정보 보호 중심 제품 분석) 및 PostHog(개인정보 보호 중심 웹사이트 분석).
...
Zush는 인공지능(AI)을 사용해 사용자가 파일을 정리, 이름 변경, 메타데이터 관리할 수 있도록 설계된 macOS 및 Windows용 데스크톱 유틸리티입니다. 기능에는 이미지, 디자인 파일, 오디오, 비디오, PDF 및 iWork를 포함한 지원 문서 자동 이름 변경, 재사용 가능한 템플릿, 145+ Naming Blocks, 스마트 메타데이터 태그(Finder 태그, Windows 파일 속성, Spotlight 메타데이터), 파일 유형 제외가 포함된 폴더 모니터링, 이름 변경 및 태그 동작을 위한 선택적 사용자 지정 AI 프롬프트 제어, macOS Finder 컨텍스트 메뉴 이름 변경, BYOK 클라우드 처리, Ollama를 통한 개인 로컬 모델 기반 오프라인 AI 모드가 포함됩니다.
...
Mac App Store 또는 Microsoft Store에서 이루어진 구매는 해당 스토어 플랫폼의 결제, 환불 및 계정 약관에 따라 처리됩니다.
...
Zush PRO는 월간 플랜 또는 일회성 구매로 제공됩니다. 구매일로부터 14일 환불 보장을 제공합니다.
...
당사는 언제든지 이 환불 정책을 수정할 권리를 보유합니다. 변경 사항은 업데이트된 "최종 업데이트" 날짜와 함께 이 페이지에 게시됩니다.`,
  ...
  'zh-cn': {
    '/privacy-policy': `本隐私政策说明 Zush（"Kirill Isachenko"、"我们"或"我们的"）在你使用我们的 macOS 和 Windows 应用及相关服务时，如何收集、使用和保护你的信息。
...
- **视觉文件**，例如图片、RAW 预览、SVG、设计文件和部分 PDF，可能会在分析前转换为压缩预览图或设计预览。
- **支持的文档**，例如文本文件、邮件、表格、演示文稿、iWork 文件和部分 PDF，可能会使用提取文本、临时 iWork PDF 预览或在你的设备上生成的紧凑内容摘要进行分析，然后再发送请求。
- **支持的音频文件** 可能会使用嵌入元数据、识别上下文、转录上下文、sidecar 转录数据，或在需要转录时使用受限音频 payload。

在正常云端操作中，除转录需要的受限音频 payload 或 iWork 分析所需的临时转换外，你的原始全分辨率文件不会离开你的设备。如果你启用离线 AI 模式，支持文件的分析会通过 Ollama 的私有本地模型运行，而不是通过 Zush 云端或第三方 AI 提供商。
...
- 电子邮件地址：在购买过程中提供。
- 许可证状态：关于你的 PRO 许可证或订阅的信息。
- 使用计数：我们跟踪 AI 处理文件数量以管理使用限制。

**1.5 支付信息**
我们不存储或处理你的信用卡详细信息。所有付款均由我们的支付处理方安全处理：[Paddle.com](https://www.paddle.com)（直接购买）、[Apple](https://www.apple.com/legal/privacy/)（Mac App Store 购买）和 [Microsoft](https://privacy.microsoft.com/privacystatement)（Microsoft Store 购买）。详情请参阅它们各自的隐私政策。

对于应用商店购买，Zush 会将商店产品和交易标识符发送到 Zush 后端，用于验证 PRO 权益、恢复购买、防止滥用并维护账户状态。
...
- **Bring Your Own Key (BYOK)：** 所有方案的用户都可以为 Google Gemini、OpenRouter、OpenAI 或 Anthropic Claude 配置自己的 API 密钥。BYOK 密钥会存储在系统的安全存储中。BYOK 模式下，密钥只会在验证密钥和处理 AI 请求时发送给 Zush 后端和所选 AI 提供商。使用 OpenRouter 时，OpenRouter 和所选模型的提供商都可能处理请求。Zush 后端不会永久存储 BYOK API 密钥。
...
**不会自动发送的内容：** 原始全分辨率文件内容和本地文件夹路径不会作为单独字段发送。但是，如果个人信息出现在文件预览、设计预览、提取文本、转录、音频 payload、临时 iWork 预览或你输入的自定义提示词中，该信息可能会作为请求的一部分被处理。

**数据保留：** 文件预览、设计预览、提取文本、临时 iWork PDF 预览、转录、音频 payload、摘要、模板、Naming Blocks 和自定义提示词文本会用于实时分析。Zush 在正常操作中不会在处理后存储文件内容，也不会故意将自定义提示词文本作为应用功能进行存储。提交的数据仍可能根据第三方 AI 提供商自己的条款被处理或保留。Zush 不会使用你的文件内容和提示词文本训练我们自己的模型。

### 4. 第三方服务

我们会与以下服务提供商共享数据：

- **AI 提供商：** Groq、Google Gemini 和 [Alibaba Cloud Model Studio（Qwen）](https://www.alibabacloud.com/help/en/model-studio/privacy-notice)。使用 BYOK 时，可使用 Google Gemini、OpenRouter 及其模型提供商、OpenAI 或 Anthropic Claude。
...
- **使用分析：** TelemetryDeck（注重隐私的产品分析）和 PostHog（注重隐私的网站分析）。
...
Zush 是一款适用于 macOS 和 Windows 的桌面工具，旨在帮助用户使用人工智能（AI）整理、重命名和管理文件元数据。功能包括对图片、设计文件、音频、视频和支持的文档（包括 PDF 与 iWork）进行自动重命名、可复用模板、145+ Naming Blocks、智能元数据标签（macOS 上的 Finder 标签、Windows 上的文件属性以及 Spotlight 元数据）、带文件类型排除的文件夹监控、用于重命名和标签行为的可选自定义 AI 提示词控制、macOS Finder 右键菜单重命名、BYOK 云端处理，以及通过 Ollama 的私有本地模型实现的离线 AI 模式。
...
通过 Mac App Store 或 Microsoft Store 完成的购买由相应商店平台按照其付款、退款和账户条款处理。
...
Zush PRO 提供月付和一次性购买两种付费方案。我们自购买日起提供 14 天退款保证。
