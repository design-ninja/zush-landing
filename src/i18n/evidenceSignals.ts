...
const EN: EvidenceSignalsCopy = {
  ...
  description:
    'Zush is an AI file renamer for Mac and Windows. It reads {count} file formats and names each file from its content, metadata, and your rules: review, batch rename, or undo anytime. Templates and folder monitoring automate repeat work; Cloud AI or Local AI decide where analysis runs.',
...
const COPY: Record<Locale, EvidenceSignalsCopy> = {
  ...
  de: {
    ...
    description:
      'Zush ist ein KI-Dateiumbenenner für Mac und Windows. Er liest {count} Dateiformate und benennt jede Datei nach Inhalt, Metadaten und Ihren Regeln: prüfen, stapelweise umbenennen oder jederzeit rückgängig machen. Templates und Ordnerüberwachung automatisieren Routinearbeit; Cloud AI oder Local AI bestimmen, wo die Analyse läuft.',
  ...
  fr: {
    ...
    description:
      'Zush est un renommeur de fichiers par IA pour Mac et Windows. Il lit {count} formats de fichiers et nomme chaque fichier d’après son contenu, ses métadonnées et vos règles : vérifiez, renommez par lot ou annulez à tout moment. Les modèles et la surveillance de dossiers automatisent les tâches répétitives ; Cloud AI ou Local AI décident où s’exécute l’analyse.',
  ...
  es: {
    ...
    description:
      'Zush es un renombrador de archivos con IA para Mac y Windows. Lee {count} formatos de archivo y nombra cada archivo según su contenido, sus metadatos y tus reglas: revisa, renombra por lotes o deshaz cuando quieras. Las plantillas y la monitorización de carpetas automatizan el trabajo repetitivo; Cloud AI o Local AI deciden dónde se hace el análisis.',
  ...
  'pt-br': {
    ...
    description:
      'O Zush é um renomeador de arquivos com IA para Mac e Windows. Ele lê {count} formatos de arquivo e nomeia cada arquivo pelo conteúdo, metadados e suas regras: revise, renomeie em lote ou desfaça quando quiser. Templates e monitoramento de pastas automatizam tarefas recorrentes; Cloud AI ou Local AI definem onde a análise é executada.',
  ...
  it: {
    ...
    description:
      'Zush è un rinominatore di file con IA per Mac e Windows. Legge {count} formati di file e rinomina ogni file in base a contenuto, metadati e regole scelte: controlla, rinomina in batch o annulla quando vuoi. Templates e monitoraggio delle cartelle automatizzano le attività ricorrenti; Cloud AI o Local AI decidono dove eseguire l’analisi.',
  ...
  nl: {
    ...
    description:
      'Zush is een AI-bestandshernoemer voor Mac en Windows. Het leest {count} bestandsformaten en benoemt elk bestand op basis van inhoud, metadata en uw regels: controleer, hernoem in batch of maak op elk moment ongedaan. Templates en mapbewaking automatiseren terugkerend werk; met Cloud AI of Local AI kiest u waar de analyse plaatsvindt.',
  ...
  tr: {
    ...
    description:
      'Zush, Mac ve Windows için yapay zekâ destekli bir dosya yeniden adlandırıcıdır. {count} dosya biçimini okur ve her dosyayı içeriğe, meta verilere ve kurallarınıza göre adlandırır: inceleyin, toplu yeniden adlandırın veya istediğiniz zaman geri alın. Templates ve klasör izleme tekrarlanan işleri otomatikleştirir; Cloud AI ya da Local AI analizin nerede çalışacağını belirler.',
  ...
  ja: {
    ...
    description:
      'Zush は Mac と Windows 向けの AI ファイルリネームツールです。{count} 種類の形式を読み取り、内容・メタデータ・ルールから各ファイル名を作成。確認して一括適用し、いつでも元に戻せます。Templates とフォルダ監視で繰り返し作業を自動化し、Cloud AI または Local AI で解析場所を選べます。',
    freeTitle: '無料で試せる',
  ...
  ko: {
    ...
    description:
      'Zush는 Mac과 Windows용 AI 파일 이름 변경 도구입니다. {count}개 형식의 파일을 읽고 콘텐츠, 메타데이터, 사용자 규칙으로 이름을 만듭니다. 검토 후 일괄 적용하고 언제든 되돌릴 수 있습니다. Templates와 폴더 모니터링으로 반복 작업을 자동화하고, Cloud AI 또는 Local AI로 분석 위치를 선택하세요.',
  ...
  'zh-cn': {
    ...
    description:
      'Zush 是一款面向 Mac 和 Windows 的 AI 文件重命名工具。它可读取 {count} 种格式，根据内容、元数据和您的规则生成文件名，检查后批量重命名，随时可撤销。Templates 和文件夹监控自动处理重复任务，Cloud AI 与 Local AI 让您选择分析位置。',
    freeTitle: '免费试用',
  ...
  ar: {
    ...
    description:
      'Zush أداة لإعادة تسمية الملفات بالذكاء الاصطناعي على Mac وWindows. يقرأ {count} صيغة ويسمّي كل ملف وفق محتواه وبياناته الوصفية وقواعدك: راجع، وأعد التسمية دفعةً واحدة، أو تراجع في أي وقت. تؤتمت Templates ومراقبة المجلدات المهام المتكررة، ويتيح Cloud AI أو Local AI اختيار مكان التحليل.',
