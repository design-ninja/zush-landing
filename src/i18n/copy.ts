...
const localizedHomeDetails: Record<Exclude<Locale, 'en'>, DeepPartial<Pick<HomeCopy, 'featureCards' | 'showcase' | 'speedComparison' | 'whyZush' | 'useCases' | 'faqItems' | 'showcaseSlides'>>> = {
  ...
  'zh-cn': {
    ...
    showcase: { title: '来一次 Zush 漫游', titleAccent: 'Zush', description: '每个功能一段简短演示 — 点击标签查看实际效果', playShowcase: '播放演示', switchTo: '切换到', items: { 'batch-rename': { title: 'AI 批量重命名', description: '根据真实内容批量检查并重命名文件后再应用' }, monitor: { title: '文件夹监控', description: '监控文件夹，自动重命名新文件' }, activity: { title: '活动历史', description: '查看最近的重命名并在需要时撤销' }, statistics: { title: '统计', description: '查看重命名数量、文件夹监控占比、活动以及文件类型趋势' }, templates: { title: '模板', description: '为 AI 重命名和文件夹监控保存可复用的设置' }, tags: { title: '智能标签', description: '生成标签，让文件搜索更快' }, naming: { title: '命名块', description: '用 145+ 个块创建可复用的文件名' }, 'naming-blocks': { title: '命名块', description: '用 145+ 个现成的命名块从文件内容生成文件名' }, multilanguage: { title: '多语言', description: '生成 60 多种语言的文件名' }, 'custom-prompts': { title: '自定义提示词', description: '用自己的指令控制文件名生成' }, byok: { title: 'BYOK', description: '连接自己的 AI 服务商，实现无限重命名' }, 'offline-ai': { title: '离线 AI 重命名', description: '对支持的文件使用 Ollama 私有本地模型' } } },
